...
export default function Comments() {
    ...
    return (
        <>
            {isAuthenticated ?
                <div style={{ padding: "1rem 2rem", backgroundColor: "#121212", borderRadius: "1rem" }}>
                    <label style={{ fontSize: "3rem", color: "white" }}>Comments</label>
                    <form action="" id='comments' name='comments'>
                ...
                : <div style={{ padding: "1rem 2rem", backgroundColor: "#121212", borderRadius: "1rem" }}>
                    <label style={{ fontSize: "3rem", color: "white" }}>Comments</label>
                    <div className="login-required">Please <button className='loginbtn' onClick={() => {
                        const returnTo = window.location.pathname.replace(/^\/Couch-Critic/, "") || "/movie";
                        localStorage.setItem("auth_return_to", returnTo);
                        loginWithRedirect();
                    }}>Login</button> to comment.</div>
                </div>}
